            ...
            try {
                ...
                if(isAvail) {
                    ...
                    if(!isNotEmpty) {
                        ...
                        // await this.$store.dispatch(POST_QCC_GROUP_ACTIVITY, formData)
                        
                        await ApiService.post(`operational/qccGroup/subActivities/add?sub_group_activity_id=${item.sub_group_activity_id}`, formData, {
                                headers: {
                        ...
                        .then(async (result) => {
                            console.log(result);
                            await Swal.fire('Berhasil upload', '', 'success')
                            this.isLoading = false
                            this.docs = []
                        }).catch((err) => {
                            console.log(err);
                            Swal.fire('Kesalahan system', '', 'error')
                        });
                    }
                }
            } catch (error) {
